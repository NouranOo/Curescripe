...
<section class="details">
    ...
    <form method="POST" action="{{ route('password.update') }}">
      ...
      <input type="hidden" name="token" value="{{ $token }}">
            ...
            <div class="group mt-3"> <input  class="password" type="email"  name="email" value="{{ $email ?? old('email') }}" required>  
            @error('email')
                    <span  role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                @enderror
            <span class="highlight"></span> <span class="bar"></span> <label>Email</label>  </div>
        <div class="group mt-3"> <input  class="password"  type="password" name="password" value="" required>  <span class="highlight">
                                @error('password')
                                    <span role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                @enderror
        </span> <span class="bar"></span> <label>Enter Password</label>  </div>
